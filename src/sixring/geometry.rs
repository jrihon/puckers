...
impl LinAlg for Coordinate {
    ...
    fn norm(&self) -> f64 {
        self.map(|x: f64| x.powi(2)).into_iter().sum::<f64>().sqrt()
    }
    ...
    fn normalise_vector(&self) -> Coordinate {
        ...
        // apply the factor `d` to all elements of the coordinate
        self.map(|x: f64| d * x)
    }
...
fn norm(c: &Coordinate) -> f64 {
    c.map(|x: f64| x.powi(2)).into_iter().sum::<f64>().sqrt()
}
...
pub fn dihedral(p0: Coordinate, p1: Coordinate, p2: Coordinate, p3: Coordinate) -> f64 {
    ...
    let v = b0.subtract_arr(&b1.scale_vector(b0.dot_product(&b1)));
    let w = b2.subtract_arr(&b1.scale_vector(b2.dot_product(&b1)));
